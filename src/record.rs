...
impl<'a> Serialize for FieldBodySerializer<'a> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error> where S: Serializer {
        match FieldType::from_tags(self.record_tag, self.prev_tag, self.field_tag, self.omwsave) {
            FieldType::I8PrefixedString => if let Field::I8PrefixedString(s) = self.field {
                ValueWithSeed(&(s.prefix, s.string.clone()), I8PrefixedStringSerde {
            ...
            FieldType::I32I8I32PrefixedString => if let Field::I32I8I32PrefixedString(s) = self.field {
                ValueWithSeed(&(s.prefix.0, s.prefix.1, s.prefix.2, s.string.clone()), I32I8I32PrefixedStringSerde {
                    code_page: self.code_page,
                }).serialize(serializer)
            } else {
                Err(S::Error::custom(format!(
                    "{} {} field should have i32 & i8 & i32 & string type", self.record_tag, self.field_tag
                )))
            },
        ...
        if deserializer.is_human_readable() && self.field_tag == META {
            ...
            match FieldType::from_tags(self.record_tag, self.prev_tag, self.field_tag, self.omwsave) {
                FieldType::I8PrefixedString =>
                    I8PrefixedStringSerde {
                        code_page: self.code_page
                    }.deserialize(deserializer).map(|(prefix, string)|
                        Field::I8PrefixedString(I8PrefixedString { prefix, string })
                    ),
                FieldType::I32I8I32PrefixedString =>
                    I32I8I32PrefixedStringSerde {
                        code_page: self.code_page
                    }.deserialize(deserializer).map(|(b0, b1, b2, string)|
                        Field::I32I8I32PrefixedString(I32I8I32PrefixedString { prefix: (b0, b1, b2), string })
                    ),
